...
def render_style_line(style: AssStyle) -> str:
    ...
    """
    return (
        f"Style: {style.name},{style.fontname},{_format_float(style.fontsize)},"
        ...
        f"{_format_float(style.scale_x)},{_format_float(style.scale_y)},"
        f"{_format_float(style.spacing)},{_format_float(style.angle)},"
        f"{style.border_style},{_format_float(style.outline)},{_format_float(style.shadow)},"
